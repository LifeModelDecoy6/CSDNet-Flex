...
def train(config):
    ...
    use_ddp = config.trainer.devices * config.trainer.num_nodes > 1
    strategy = 'auto'
    if use_ddp:
    ...
    if config.eval.run_validation:
        val_dataloader = get_dataloader(config, split='validation')
        if val_dataloader is not None:
            trainer.validate(model=model, dataloaders=val_dataloader, ckpt_path=None)
    
    if config.eval.run_test:
        test_dataloader = get_dataloader(config, split='test')
        if test_dataloader is not None:
            trainer.test(model=model, dataloaders=test_dataloader, ckpt_path=None)
