...
class DockingVina(object):
    def __init__(self, target):
        ...
        tmp_name = f"tmp_{target}_{job_id}_{os.getpid()}_{uuid.uuid4().hex[:8]}"
        tmp_dir = os.path.join(ROOT_DIR, 'docking/tmp', tmp_name)
    ...
    def predict(self, smiles_list):
        ...
        try:
            q1.close()
            q1.join_thread()
            proc_master.join(timeout=60)
            if proc_master.is_alive():
                proc_master.terminate()
                proc_master.join(timeout=5)

            molecules_per_worker = ceil(
                max(1, len(data)) / max(1, self.num_sub_proc)
            )
            worker_timeout = max(
                180,
                molecules_per_worker
                * (self.timeout_gen3d + self.timeout_dock + 15)
                + 60,
            )
            deadline = monotonic() + worker_timeout
            for proc in procs:
                proc.join(timeout=max(0.0, deadline - monotonic()))

            timed_out = [proc for proc in procs if proc.is_alive()]
        ...
        missing = len(data) - len(results)
        if timed_out or missing:
            print(
                "Docking worker watchdog: "
                f"timed_out_workers={len(timed_out)} "
                f"missing_results={missing}/{len(data)}"
            )
        return [results.get(idx, 99.9) for idx in range(len(data))]
