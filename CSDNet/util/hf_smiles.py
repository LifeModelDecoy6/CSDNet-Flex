...
def resolve_hf_token(token=None, token_env="HF_TOKEN", required=False):
    ...
    if required and not resolved:
        raise SystemExit(f"Missing HuggingFace token. Set {token_env} or pass --hf_token.")
    return resolved
...
def extract_smiles_from_row(row, smiles_col="auto", safe_col="safe", allow_safe_decode=True):
